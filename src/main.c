...
char ** calculate_area(double a, double b, int partition_sizes[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        char *output = malloc(sizeof(char) * MAX_STRING_SIZE_);
...
int main()
{
    ...
    double a, b;
    scanf("%lf %lf", &a, &b);
    char **outputs = calculate_area(a, b, partition_sizes, size_of_array);
    for (int i = 0; i < size_of_array; i++)
    {
        printf("%s\n", outputs[i]);
    }

    free_outputs(outputs, size_of_array);
