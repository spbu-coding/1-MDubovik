...
char ** calculate_area(double a, double b, int partition_sizes[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < partition_sizes[i]; j++) {
            area_by_rectangle += rectangle_rule(left_border, right_border);
            area_by_simpsons += simpsons_rule(left_border, right_border);
...
int main()
{
    ...
    double a, b;
    scanf("%lf %lf", &a, &b);
    char **outputs = calculate_area(a, b, partition_sizes, 6);
    for (int i = 0; i < 6; i++)
    {
        printf("%s\n", outputs[i]);
    }

    free_outputs(outputs, 6);
